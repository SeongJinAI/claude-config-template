...
/notion pending                                        # 전체 메뉴 미완료 항목 조회
/notion pending <메뉴>                                 # 특정 메뉴 미완료 항목 조회
```
...
- **제품 기능 변경만** 기록한다. 내부 도구/워크플로 설정(에이전트 가이드, CI 설정, 코드 리뷰 프로세스 등)은 제외.
- **소급 기록**: 마지막 Notion 기록일 ~ 명령 시점 사이에 누락된 제품 기능 작업은 핸드오프/git 이력에서 확인하여 함께 기록한다.
- Notion은 **기획자-개발자 소통용**이므로, 최종 사용자에게 의미 있는 변경만 기록한다.
...
### pending — 미완료 항목 조회 (세션 시작용)

세션 시작 시 남은 작업을 파악하기 위해 미체크(`checked == false`) 항목만 조회합니다.

1. 메뉴 지정 시: 해당 toggle 하위 to_do 중 `checked == false`만 필터링
2. 메뉴 미지정 시: 모든 메뉴 toggle을 순회하며 미체크 항목 수집, 메뉴별 그룹핑
3. API: `GET /blocks/{BLOCK_ID}/children` → 클라이언트에서 `checked == false` 필터링 (Notion API 자체 필터 미지원)
4. 페이지네이션: 응답의 `has_more == true`이면 `start_cursor`로 추가 조회
5. 미완료 항목이 없는 메뉴는 출력에서 생략
